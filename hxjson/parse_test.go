...
func TestEncodeConcatenatedHashes(t *testing.T) {
	...
	for j := 0; j < len(hashSlice)+1; j++ {
		...
		// Encode to string
		concatenated := hxjson.EncodeConcatenatedHashes(hashSlice[:j])
		// Verify output
...
func TestDecodeConcatenatedHashes(t *testing.T) {
	...
	decodedHashes, err := hxjson.DecodeConcatenatedHashes(concatenatedHashes)
	if err != nil {
...
func TestEncodeConcatenatedVoteBits(t *testing.T) {
	...
	encodedResults, err := hxjson.EncodeConcatenatedVoteBits(testVbs)
	if err != nil {
	...
	_, err = hxjson.EncodeConcatenatedVoteBits(testVbs)
	if err == nil {
...
func TestDecodeConcatenatedVoteBits(t *testing.T) {
	...
	decodedSlice, err :=
		hxjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err != nil {
	...
	_, err = hxjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err == nil {
...
func TestInvalidDecodeConcatenatedHashes(t *testing.T) {
	testStrings := []struct {
	...
	}{
		{
			...
			hxjson.RPCError{
				Code: hxjson.ErrRPCDecodeHexString,
			},
		}, {
			// invalid length
