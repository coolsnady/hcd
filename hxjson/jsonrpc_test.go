...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if hxjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *hxjson.RPCError {
				return hxjson.NewRPCError(hxjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := hxjson.MarshalResponse("1.0", testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := hxjson.NewRequest("1.0", nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := hxjson.Error{Code: hxjson.ErrInvalidType}
	_, err = hxjson.MarshalResponse("", make(chan int), nil, nil)
	if jerr, ok := err.(hxjson.Error); !ok || jerr.Code != wantErr.Code {
	...
	// can't be marshalled.
	_, err = hxjson.MarshalResponse("1.0", 1, make(chan int), nil)
