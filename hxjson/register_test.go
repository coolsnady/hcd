...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		...
		{0, "0x0"},
		{hxjson.UFWalletOnly, "UFWalletOnly"},
		{hxjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{hxjson.UFNotification, "UFNotification"},
		{hxjson.UFWalletOnly | hxjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{hxjson.UFWalletOnly | hxjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := hxjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: hxjson.Error{Code: hxjson.ErrDuplicateMethod},
		},
		{
			...
			flags: hxjson.TstHighestUsageFlagBit,
			err:   hxjson.Error{Code: hxjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: hxjson.Error{Code: hxjson.ErrEmbeddedType},
		},
		{
			...
			err: hxjson.Error{Code: hxjson.ErrUnexportedField},
		},
		{
			...
			err: hxjson.Error{Code: hxjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: hxjson.Error{Code: hxjson.ErrNonOptionalField},
		},
		{
			...
			err: hxjson.Error{Code: hxjson.ErrNonOptionalDefault},
		},
		{
			...
			err: hxjson.Error{Code: hxjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := hxjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	hxjson.MustRegisterCmd("panicme", 0, 0)
}
...
func TestRegisteredCmdMethods(t *testing.T) {
	...
	methods := hxjson.RegisteredCmdMethods()
	if len(methods) == 0 {
