...
func ExampleMarshalCmd() {
	...
	// optional fields.  Also, notice the call to hxjson.Bool which is a
	// convenience function for creating a pointer out of a primitive for
	// optional parameters.
	blockHash := "000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"
	gbCmd := hxjson.NewGetBlockCmd(blockHash, hxjson.Bool(false), nil)
	...
	id := 1
	marshalledBytes, err := hxjson.MarshalCmd("1.0", id, gbCmd)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleUnmarshalCmd() {
	...
	gbCmd, ok := cmd.(*hxjson.GetBlockCmd)
	if !ok {
...
func ExampleMarshalResponse() {
	...
	marshalledBytes, err := hxjson.MarshalResponse("1.0", 1, 350001, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
