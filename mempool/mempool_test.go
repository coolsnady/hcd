...
type fakeChain struct {
	...
	blocks        map[chainhash.Hash]*hxutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *hxutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*hxutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *hxutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *hxutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
// transactions.
func txOutToSpendableOut(tx *hxutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   hxutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*hxutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32) (*hxutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput hxutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*hxutil.Tx, error) {
	txChain := make([]*hxutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, hxutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := hxutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*hxutil.Block),
...
func testPoolMembership(tc *testContext, tx *hxutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
