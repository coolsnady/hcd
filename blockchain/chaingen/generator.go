...
func (s *SpendableOut) Amount() hxutil.Amount {
	return s.amount
}
...
func makeSpendableOutForTxInternal(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32, tree int8) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      hxutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func MakeGenerator(params *chaincfg.Params) (Generator, error) {
	...
	p2shOpTrueAddr, err := hxutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
// a single OP_TRUE.
func (g *Generator) P2shOpTrueAddr() hxutil.Address {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) hxutil.Amount {
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy hxutil.Amount, blockHeight uint32, numVotes uint16) hxutil.Amount {
	powProportion := hxutil.Amount(g.params.WorkRewardProportion)
	totalProportions := hxutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := hxutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * hxutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) hxutil.Amount {
	if int64(heightVotedOn+1) < g.params.StakeValidationHeight {
		return 0
	}

	fullSubsidy := g.calcFullSubsidy(heightVotedOn)
	posProportion := hxutil.Amount(g.params.StakeRewardProportion)
	totalProportions := hxutil.Amount(g.params.TotalSubsidyProportions())
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy hxutil.Amount, blockHeight uint32, numVotes uint16) hxutil.Amount {
	devProportion := hxutil.Amount(g.params.BlockTaxProportion)
	totalProportions := hxutil.Amount(g.params.TotalSubsidyProportions())
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := hxutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * hxutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy hxutil.Amount) {
...
func PurchaseCommitmentScript(addr hxutil.Address, amount, voteFeeLimit, revocationFeeLimit hxutil.Amount) []byte {
...
func (g *Generator) CreateTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee hxutil.Amount) *wire.MsgTx {
...
func (g *Generator) CreateVoteTx(voteBlock *wire.MsgBlock, ticketTx *wire.MsgTx, ticketBlockHeight, ticketBlockIndex uint32) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / hxutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := hxutil.Amount(ticketTx.TxOut[0].Value)
...
// will be nil.
func buildMerkleTreeStore(transactions []*hxutil.Tx) []*chainhash.Hash {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*hxutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, hxutil.NewTx(tx))
	}
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee hxutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee hxutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := hxutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = hxutil.Amount(2)
	...
	var stakeTreeFees hxutil.Amount
	for txIdx, tx := range stakeTxns {
		...
		var inputSum, outputSum hxutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += hxutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += hxutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount hxutil.Amount, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	var totalSubsidy hxutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		payoutAddr, err := hxutil.DecodeAddress(payout.Address)
		if err != nil {
			panic(err)
		}
		...
		totalSubsidy += hxutil.Amount(payout.Amount)
	}
