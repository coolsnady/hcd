...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []hcutil.Address, int, error) {
	if version != DefaultScriptVersion {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := hcutil.NewAddressPubKeyHash(pops[2].data,
	...
	case PubKeyTy:
		...
		if err == nil {
			addr, err := hcutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	case PubkeyAltTy:
		...
		switch suite {
		case chainec.ECTypeEdwards:
			addr, err = hcutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case chainec.ECTypeSecSchnorr:
			addr, err = hcutil.NewAddressSecSchnorrPubKey(pops[0].data,
				chainParams)
		case bs.BSTypeBliss:
			addr, err = hcutil.NewAddressBlissPubKey(pops[0].data,
				chainParams)
		}
		if err == nil {
			addrs = append(addrs, addr)
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := hcutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
...
//
// NOTE: Atomic swaps are not considered standard script types by the hcd
