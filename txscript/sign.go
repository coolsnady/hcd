...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []hcutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
	// No need to add dummy in Hcd.
...
func handleStakeOutSign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	addresses []hcutil.Address, class ScriptClass, subClass ScriptClass,
	nrequired int) ([]byte, ScriptClass, []hcutil.Address, int, error) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []hcutil.Address, int, error) {
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []hcutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		if err != nil {
			// Hcd -- is this the right handling for SIGHASH_SINGLE error ?
...
type KeyDB interface {
	GetKey(hcutil.Address) (chainec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(hcutil.Address) (chainec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address hcutil.Address) (chainec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(hcutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(hcutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address hcutil.Address) ([]byte, error) {
	return sc(address)
}
