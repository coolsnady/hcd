...
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a hcutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "DsSej1qR3Fyc8kV176DCh9n9cY9nqf9Quxk"
	address, err := hcutil.DecodeAddress(addressStr)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	pubKeyHash := hcutil.Hash160(pubKey.SerializeCompressed())
	...
	// contains a single output that pays to address in the amount of 1 HC.
	...
	lookupKey := func(a hcutil.Address) (chainec.PrivateKey, bool, error) {
