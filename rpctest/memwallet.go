...
type memWallet struct {
	...
	addrs map[uint32]hcutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]hcutil.Address)
...
func (m *memWallet) IngestBlock(header []byte, filteredTxns [][]byte) {
	...
	txns := make([]*hcutil.Tx, 0, len(filteredTxns))
	for _, txBytes := range filteredTxns {
		tx, err := hcutil.NewTxFromBytes(txBytes)
		if err != nil {
			panic(err)
		}
		txns = append(txns, tx)
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash, isCoinbase bool, undo *undoEntry) {
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          hcutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (hcutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []hcutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt hcutil.Amount, feeRate hcutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := hcutil.Amount(txSize * int(feeRate))
...
// in atoms-per-byte.
func (m *memWallet) SendOutputs(outputs []*wire.TxOut, feeRate hcutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate hcutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt hcutil.Amount
	for _, output := range outputs {
		outputAmt += hcutil.Amount(output.Value)
		tx.AddTxOut(output)
	}

	// Attempt to fund the transaction with spendable utxos.
	if err := m.fundTx(tx, outputAmt, hcutil.Amount(feeRate)); err != nil {
		return nil, err
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() hcutil.Amount {
	m.RLock()
	defer m.RUnlock()
...
func keyToAddr(key chainec.PrivateKey, net *chaincfg.Params) (hcutil.Address, error) {
	pubKey := chainec.Secp256k1.NewPublicKey(key.Public())
	serializedKey := pubKey.SerializeCompressed()
	pubKeyAddr, err := hcutil.NewAddressSecpPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
