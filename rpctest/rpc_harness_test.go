...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt hxutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(hxutil.Amount(5 * hxutil.AtomsPerCoin))
	...
	txid = genSpend(hxutil.Amount(5000 * hxutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(hxjson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(hxjson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := hxutil.Amount(5 * 300 * hxutil.AtomsPerCoin)
	...
	// chain.
	expectedBalance = hxutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := hxutil.Amount(50 * hxutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := hxutil.Amount(numMatureOutputs * 300 * hxutil.AtomsPerCoin)
