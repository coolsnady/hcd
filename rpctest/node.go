...
// nodeConfig contains all the args, and data required to launch a hxd process
...
// arguments returns an array of arguments that be used to launch the hxd
// process.
...
// command returns the exec.Cmd which will be used to start the hxd process.
...
// to the hxd process that is launched via Start().
...
// as the base for the log and data directories for hxd.
...
// start creates a new hxd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "hxd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running hxd process process, and waits until it exits
...
// shutdown terminates the running hxd process, and cleans up all
