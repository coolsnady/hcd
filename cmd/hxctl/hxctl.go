...
func main() {
	...
	method := args[0]
	usageFlags, err := hxjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := hxjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// hxjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(hxjson.Error); ok {
		...
		// The error is not a hxjson.Error and this really should not
	...
	marshalledJSON, err := hxjson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
